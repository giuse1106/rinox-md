...
let handler = async (m, { conn, usedPrefix, command, text }) => {
    ...
    if (Object.values(conn.game).find(room => room.id.startsWith('chess') && [room.game.white, room.game.black].includes(m.sender))) {
        return m.reply('*[❗] _STAI GIA GIOCANDO UNA PARTITA DI SCACCHI_*');
    }
    ...
    if (room) {
        await m.reply('♟️ *UN GIOCATORE SI È UNITO, INIZIO DELLA PARTITA!*');
        room.black = m.chat;
...
`.trim();
        ...
        conn.reply(m.chat, `♟️ *PARTITA DI SCACCHI CREATA!* ♟️
